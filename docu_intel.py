...
def generate_text_insights(text_content, visual_slides):  
    headers = {  
        "Content-Type": "application/json",  
        "api-key": api_key  
    }  
...
def generate_image_insights(image_content):  
    ...
    for image_data in image_content:  
        ...
        explanation = get_image_explanation(base64_image)  
        if explanation:  
            insights.append({"slide_number": image_data['slide_number'], "slide_title": image_data.get('slide_title', 'Untitled Slide'), "insight": explanation})  
    return insights  
...
def extract_and_clean_page_image(page):  
    ...
    for x, y, w, h in valid_contours:  
        # Increase the size of the bounding box by 20 pixels on each side  
        cv2.rectangle(mask, (x - 85, y - 70), (x + w + 85, y + h + 70), 255, -1)  
...
def extract_images_from_pdf(pdf_file):  
    ...
    for page_num in range(len(pdf_document)):  
        ...
        cleaned_image = extract_and_clean_page_image(page)  
...
def main():  
    ...
    if uploaded_ppt is not None:  
        ...
        text_insights = generate_text_insights(text_content, visual_slides)  
  
        st.info("Generating image insights...")  
        image_insights = generate_image_insights(slide_images)  
  
        st.info("Extracting additional images...")  
        extracted_images = extract_images_from_pdf("temp_pdf.pdf")  
